...
import { seedLaunchReferenceProfiles } from "./seed-launch-reference-profiles";
import { validateLaunchReferenceDatabase } from "./validate-launch-reference-profiles";
import {
...
async function deleteLaunchReferenceProfiles(client: DbClient) {
...
async function main() {
  ...
  try {
    ...
    const deleted = await deleteLaunchReferenceProfiles(client);
    const seeded = await seedLaunchReferenceProfiles(client);
    const validation = await validateLaunchReferenceDatabase(client);

    if (!validation.ok) {
      throw new Error(
        `Launch reference validation failed after refresh:\n${validation.errors.join("\n")}`
      );
    }

    await client.query("COMMIT");
    console.log("[api-server] launch reference profiles refreshed", {
